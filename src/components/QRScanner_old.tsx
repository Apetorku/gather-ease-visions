...
import { Camera, CheckCircle2, XCircle, Search, Users, Clock, TrendingUp, Download, Filter } from "lucide-react";
...
export const QRScanner = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveStats(prev => ({
        ...prev,
        checkedIn: Math.min(prev.totalRegistered, prev.checkedIn + Math.floor(Math.random() * 3)),
  ...
  const handleManualCheck = () => {
    if (manualEntry) {
      ...
      const isDuplicate = scanHistory.some(scan => 
        scan.ticketNumber === manualEntry && scan.status === "valid"
      );
      
      const newScan: ScanResult = {
        ticketNumber: manualEntry,
        attendeeName: isValidTicket ? `User ${Math.floor(Math.random() * 1000)}` : "Invalid User",
        ticketType: manualEntry.includes('VIP') ? 'VIP Pass' : 'Standard',
        status: isDuplicate ? "already-used" : (isValidTicket ? "valid" : "invalid"),
        timestamp: new Date().toLocaleTimeString(),
      };
  ...
  const exportData = () => {
    const csvContent = [
      ...
      ...scanHistory.map(scan => [
        scan.ticketNumber, scan.attendeeName, scan.ticketType, scan.status, scan.timestamp
      ])
      ...
      <div className="grid md:grid-cols-4 gap-4 mb-6">
        <GlassCard className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-bold text-primary">{liveStats.totalRegistered}</p>
        ...
        <GlassCard className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-bold text-green-500">{liveStats.checkedIn}</p>
        ...
        <GlassCard className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-bold text-blue-500">{liveStats.checkInRate}%</p>
        ...
        <GlassCard className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-2xl font-bold text-purple-500">{new Date().toLocaleTimeString()}</p>
